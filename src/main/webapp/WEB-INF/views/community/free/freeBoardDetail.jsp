...
<body>
	<div class="col-lg-6 col-md-6 col-sm-6 col-xs-12">
		...
		<form action="http://localhost/www/freeModifyFormT" method="get">
			...
			<div class="freeDetailContent">
				<div class="freeDetailContentTop">
					...
					<div id="freeDetail2" class="col-lg-3">
						<input type="text" class="form-control text-center"
							value="${dto.bod_id}(${dto.bod_type})" readonly="readonly" />
					</div>
				...
				<div class="freeDetailContent2">
					<div style="width: 1220px;">${dto.bod_content}</div>
				</div>
				<br>

			</div>
			<c:if test="${id eq dto.bod_id}">
				<div class="freeDetailContentButton">
					<div align="right" style="margin-top: 4%;">
						<input align="right" type="button" class="btn btn-sm btn-freeAdd"
							value="목록보기"
							onclick="location.href='http://localhost/www/freeBoardListT'" />
						<input align="right" type="submit" class="btn btn-sm btn-freeAdd"
							value="수정하기" /> <input align="right" type="button"
							class="btn btn-sm btn-freeAdd" value="삭제하기"
							onclick="deleteFreeCheck()" />
					</div>
				</div>
			</c:if>
		...
		<c:choose>
			<c:when
				...
				<div class="freeDetailComment">
					<form name="freeReply" action="http://localhost/www/freeReply"
						method="post">
						<input type="hidden" value="${bod_num}" name="bod_num" /> <input
							type="hidden" value="${id}" name="cf_id" />
						<textarea style="background: rgba(51, 102, 0, 0.4); resize: none;"
							name="cf_content" rows="5" cols="140" required="required"
							placeholder="여기에 댓글 내용을 쓰시면 됩니다. 댓글은 나의 얼굴입니다"></textarea>
						<div align="right" style="margin-top: 4%;">
							<input type="submit" class="btn btn-sm btn-freeAdd" value="댓글쓰기" />
							<input type="reset" class="btn btn-sm btn-freeAdd" value="지우기" />
						</div>
					</form>
				</div>
			</c:when>
		...
		<c:if test="${freeReplyList.size()>0}">
			<div class="freeDetailComment">
				<c:forEach var="list" items="${freeReplyList}">
					<form name="freeModifyForm"
						action="http://localhost/www/go_modifyPageFree" method="post">
						...
						<div class="col-lg-2">
							<input style="margin-left: -8%; margin-top: -4%"
								...
								value="${list.cf_id}(${list.cf_type})" readonly="readonly" />
						</div>
						<div class="col-lg-3">
							<input style="margin-left: -8%; margin-top: -2%"
								class="form-control" type="text" value="${list.cf_join}"
								readonly="readonly" />
						</div>
						<c:if test="${type.equals('admin')}">

							<input type="button" class="btn btn-sm btn-freeAdd" value="댓글삭제"
								onclick="deleteFreeCheck2(${list.cf_num},${list.bod_num})" />
						</c:if>

						<!-- 관리자 삭제 -->
						<c:if test="${list.cf_id.equals('admin')&&type.equals('admin')}">

							<input type="submit" class="btn btn-sm btn-freeAdd" value="댓글수정" />
						</c:if>

						<!-- 관리자 수정 -->
						<c:if test="${list.cf_id eq id &&type.equals('user')}">
						...
						<c:if test="${list.cf_id eq id &&type.equals('club')}">
						...
						<c:choose>
							<c:when
								test="${list.cf_num==old_cf_num&&modifyFocusAble.equals('true')}">
								<textarea
									style="background: rgba(51, 102, 0, 0.4); resize: none;"
									autofocus="autofocus" rows="5" cols="140" readonly="readonly"
									name="cf_content">${list.cf_content}</textarea>
							</c:when>
							<c:otherwise>
								<textarea
									style="background: rgba(51, 102, 0, 0.4); resize: none;"
		...
		<script type="text/javascript">
			function deleteFreeCheck() {
				var checkFlag = window.confirm("정말 삭제하시겠습니까?");
				if (checkFlag) {
					location.href="http://localhost/www/freeDelete?bod_num=${dto.bod_num}";
				}
			}
			
			function deleteFreeCheck2(cf_num,bod_num) {
				var checkFlags = window.confirm("정말 삭제하시겠습니까?");
				if (checkFlags) {
					location.href="http://localhost/www/deleteFreeReply?cf_num="+cf_num+"&bod_num="+bod_num;
				}
			}
