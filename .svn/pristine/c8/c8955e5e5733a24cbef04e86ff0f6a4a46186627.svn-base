		...
		function ajax_changeDays(){
			...
			$.get(
				...
				function(res){
					...
					alert(res.mat_reDTO.mat_num);
					
					/* 방법1 - 마지막날만 받아와서 for을 따로 돌려준다.
					alert("res.lastDay : "+res.lastDay);
					var newDays="";
					for(var i=0;i<res.lastDay;i++){
						newDays+="<option value="+(i+1)+">"+(i+1)+"</option>";
					} 
 					alert(newDays);
					$("select[name=day]").html(newDays); 
					*/
					
					/* 방법2 - 이미 돌려놓은 for문을 받아와서 select박스 안에 집어넣기만 한다. 
					alert(res.days_list);
					*/
					$("select[name=day]").html(res.days_list);
				},
				"json"
			);
		}//ajax_changeDays()
				...
				<c:forEach var="fors" items="${months }" >
					<c:choose>
						<c:when test="${fors.equals(date.month)}">
							<option value="${fors }" selected="selected" onchange="ajax_changeDays();">${fors }</option>
						</c:when>
						<c:otherwise>
							<option value="${fors }" onchange="ajax_changeDays();">${fors }</option>
						</c:otherwise>
					</c:choose>
				</c:forEach>
			</select>
